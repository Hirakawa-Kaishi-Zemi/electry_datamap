                    ...
                    dbc.Col(
                        [html.Div([
                            ...
                            dcc.Dropdown(id='my-corr-picker', multi=True,
                                         ...
                                         options=[{'label': x, 'value': x}
                                                  for x in company],
                                         style={'width': '320px',  # 'color': '#8800e3','border-color': '#8800e3',#'background-color':'#9d18f5',
                                         }
                                         )
...
#labelの作成が必要　凡例をつくる
    fig = make_subplots(
        ...
        specs=[[{"type": "pie"}, {"type": "pie"},{"type": "pie"},{"type": "pie"},{"type": "pie"}]],  # subplotするグラフの配置順やグラフの種類
...
#labelの作成が必要　凡例をつくる
    fig = make_subplots(
        rows=1, cols=5,  # 行数と列数
        vertical_spacing=0.1,  # 表とグラフの間の間隔を0に
        specs=[[{"type": "bar"}, {"type": "bar"},{"type": "bar"},{"type": "bar"},{"type": "bar"}]],  # subplotするグラフの配置順やグラフの種類
        subplot_titles=['東京電力', '関西電力', '中部電力', '東北電力','中国電力'],
        )
